...
if csv_file:
    ...
    if all(f in df.columns for f in features):
        ...
        top_players = df.sort_values(by='value_gap', key=abs, ascending=False).head(40)
        st.success("Top 30–40 Value Bets Based on Predictions:")
        st.dataframe(top_players[["player", "team", "opponent", "predicted_points", "line", "value_gap", "value_rating"]])
    else:
